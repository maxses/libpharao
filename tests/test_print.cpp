...
TEST_CASE( "Print", "[default]" )
{
   ...
   SECTION( "Print" )
   {
      ...
      pharao_snprintf( dest, sizeof(dest), "%d", 1234567890 );
      REQUIRE ( str == dest );
      REQUIRE ( str != "Hallo Welt" );

      memset(dest, '#', sizeof(dest));
      str=QString("%1").arg(0x1234AFFE, 0, 16).toUpper();
      pharao_snprintf( dest, sizeof(dest), "%x", 0x1234AFFE );
      REQUIRE ( str == dest );
      ...
      size=pharao_snprintf( dest, sizeof(dest), "X%dX", -5129988 );
      REQUIRE ( str == dest );

      memset(dest, '#', sizeof(dest));
      str=QString("XMUHX 99088 ");
      size=pharao_snprintf( dest, sizeof(dest), "X%c%c%cX %d ", 'M', 'U', 'H', 99088 );
      REQUIRE ( str == dest );
